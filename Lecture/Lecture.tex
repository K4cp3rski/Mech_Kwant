...
\begin{lecture}{Zaburzenia w czasie}
    ...
    \emph{Przykład}: Zaburzenie harmoniczne\\
    \[
        \hat{V}(t) = 2 \cdot \hat{V} \cdot \sin(\omega t)  
    \]
    Liczymy:
        \[
            a_n^{(1)}(t) = \qty(-\frac{i}{\hbar}) e^{- \frac{i E_n t}{\hbar}} \mel{n}{V}{i} \underbrace{2 \int \dd{t'} \sin(\omega t') e^{\frac{i t' (E_n - E_i)}{\hbar}}}_{A} 
        \]
    Zaś $\omega_{ni} = \frac{(E_n - E_i)}{\hbar}$
    
    Teraz $A$ policzymy też jako:
    \[
        A = \dots = 2 i \qty[e^{\frac{i (\omega_{ni} + \omega) t}{2}} \frac{\sin(\frac{\omega_{ni} + \omega}{2} t)}{\omega_{ni} + \omega} - e^{\frac{i (\omega_{ni} - \omega) t}{2}} \frac{\sin(\frac{\omega_{ni} - \omega}{2} t)}{\omega_{ni} - \omega}]  
    \]

    Duże amplitudy będą w okolicach $\omega_{ni} = \pm \omega$.\\
    Jeśli $\omega_{ni} = \omega$ (absorpcja) to dominujący człon w prawdopodobieństwie to 
    \[
        \abs{a_n^{(1)}(t)} \approx \frac{4}{\hbar^2} \abs{\mel{n}{V}{i}}^2 \frac{\sin^2\qty(\frac{\omega_{ni} - \omega}{2} t)}{\qty(\omega_{ni} - \omega)^2}
    \]
    Czyli możemy wprowadzić sobie szybkość przejścia:
    \[
        W_{i\to n} = \frac{2 \pi}{\hbar^2} \abs{\mel{m}{V}{i}}^2 \delta(\omega_{ni} - \omega)  
    \]
    Co jest znane jako \subind{złota reguła Fermiego}{Złota Reguła Fermiego}

    \section{\subind{Twierdzenie Adiabatyczne}{Twierdzenie!Adiabatyczne}}
    $H(t)$ - bardzo wolno zmienny w czasie i pytamy jak będzie ewoluował $\psket{t}$?\\
    Oznaczamy stany i energie własne w chwili $t$ jako: $H(t) \ket{n(t)} = E_n(t) \ket{n(t)}$.\\
    Dwolone rozwiązanie możemy zawsze zapisać jako: $\psket{t} = \sum_n c_n(t) e^{9 \theta_n(t)} \ket{n(t)}$, $\theta_n(t) = - \frac{1}{\hbar} \int_0^t \dd{t'} E_n(t')$.\\
    Wstawmy sobie to teraz do równania Schrödingera:
    \[
        i \hbar \dv{t} \psket{t} = H(t) \psket{t}  
    \]
    Co daje dalej:
    \begin{align*}
        i \hbar \sum_n \qty[\dot{c}_n \ket{n(t)} \hcancel{$- \frac{i}{\hbar E_n(t) c_n(t) \ket{n(t)}}$} + c_n(t) \ket{\dot{n}(t)}]e^{- \theta_n(t)} &= \hcancel{$\sum_n c_n(t) e^{\theta_n(t)} E_n(t) \ket{n} $} \\
        \sum_n \dot{c}_n e^{\theta_n(t)} \ket{n(t)} + c_n(t) e^{i \theta_n(t)} \ket{\dot{n}(t)} &= 0 \\
        \dot{c}_m(t) &= - \sum_n c_n (t) \braket{m(t)}{\dot{n}(t)} e^{i(\theta_n(t) - \theta_m(t))} \\
        H(t) \ket{n(t)} &= E_n(t) \ket{n(t)} / \dv{t} \\
        \bra{m(t)} | \dot{H}(t){n(t)} + H(t) \ket{\dot{n}(t)} &= \hcancel{$\dot{E}_n (t) \ket{n(t)}$} + E_n (t) \ket{\dot{n}(t)} \\
        \mel{m(t)}{\dot{H}(t)}{n(t)} + \underbrace{\mel{m(t)}{H(t)}{\dot{n}(t)}}_{E_n(t) \braket{m(t)}{n(t)}} = E_n(t) \braket{m(t)}{\dot{n}(t)}
    \end{align*}
    Gdzie $m \neq n$ oraz 
    $$\braket{m(t)}{\dot{n}(t)} = \frac{\mel{m(t)}{\dot{H}}{n(t)}}{E_n(t) - E_m(t)}$$
    Pamiętając, że
    \[
       \dot{c}_m(t) = - c_m(t) \braket{m(t)}{\dot{m}(t)} - \sum_{m \neq n} c_n(t) e^{i \qty[\theta_n(t) - \theta_m(t)]} \cdot \frac{\mel{m(t)}{\dot{H}}{n(t)}}{E_n(t) - E_m(t)}
    \]
    Gdzie ten drugi człon odpowiada za 'przeskoki' pomiędzy poziomami energetycznymi. Jeśli jest to bardzo małe,, to jeśli stan początkowy był stanem własnym $\psket{0} = \ket{m=0}$, to pozostanie cały czas stanem własnym $H(t): \psket{t} = \ket{m(t)}$
\end{lecture}
...
\begin{lecture}{Zaburzenia w czasie vol.2}
    ...
    Czyli ostatecznie dostajemy, że:
    \[
        \psket{t} = \underbrace{e^{i \theta_m(t)}}_{\footnotemark} \underbrace{e^{i \gamma(t)}}_{\footnotemark} \ket{m(t)}
    \]
    \addtocounter{footnote}{-1}
    \footnotetext{Faza Dynamiczna}
    \refstepcounter{footnote}
    \footnotetext{Faza Geometryczna}
    Z warunkiem, że $\psket{0} = \ket{m(0)}$. Człon pierwszy to \subind{faza dynamiczna}{Faza!Dynamiczna}, a drugi to \subind{faza geometryczna}{Faza!Geometryczna}.
    Fazę Geometryczną nazywamy inaczej \emph{Fazą Berry'ego}, ale do tego jeszcze dojdziemy.\\
    Uwaga ad fazy:\\
    Jeśli przedefiniujemy stany własne:
    \[
        \ket{m(t)} \to e^{i \chi(t)} \ket{m(t)}
    \]
    Oraz wiemy też, że przechodzi to jak:
    \[
        \gamma(t) \to \gamma(t) + i \int_0^t \dd{t'} \mel{m(t')}{i\dot{\chi}(t)}{m(t')} = \gamma(t) - \int_0^t \dd{t'} \dot{\chi}(t') 
    \]
    Teraz usuwamy sobie bezpośrednia zależność od czasu z Hamiltonianu i dostajemy:
    \[
        H(\vb{R}(t))  
    \]
    Gdzie $\vb{R}(t)$ to zestaw zależnych od czasu parametrów Hamiltonianu.
    Dajemy sobie warunek:
    \[
        \vb{R}(t) = \vb{R}(0)    
    \]
    Rozpiszmy sobie $\gamma$:
    \begin{equation} \label{eq:lec_21:berry_phase}
        \gamma(t) = i\int_0^t \dd{t'} \braket{m(t)}{\dot{m}(t)} = i \int_{\vb{R}(0)}^{\vb{R}(t)} \dd{\vb{R}} \mel{m(\vb{R})}{\grad_{\vb{R}}}{m(\vb{R})} \stackrel{\vb{R(t)} = \vb{R(0)}}{=} i \oint_C \dd{\vb{R}} \mel{m(\vb{R})}{\grad_{\vb{R}}}{m(\vb{R})} =: \gamma_C
    \end{equation}
    Gdzie teraz konkretnie sformułowanie $\gamma_C$ nazywamy \subind{fazą Berry'ego}{Faza!Berry'ego}.
    Wiemy o niej też, że jak idziemy sobie po zamkniętej trajektorii w przestrzeni parametrów, to ona ewoluuje jak:
    \[
        \gamma_C \to \gamma_C - \oint_C \dd{\vb{R}} \cdot \grad{\chi(\vb{R})}    
    \]
    Ale $\oint_C \dd{\vb{R}} \cdot \grad{\chi(\vb{R})} $ jest po konturze zamkniętym, więc $\oint_C \dd{\vb{R}} \cdot \grad{\chi(\vb{R})}  = 0$

    \section{Macierz Gęstości, stany splątane i dekoherencja}

    $\ket{\psi}$ - stan czysty (pełna wiedza o układzie), $\ket{\Psi} \in \HS$.\\
    Chcemy opisać układ o którym nie mamy pełnej wiedzy.\\
    Załóżmy, że wiemy, że układ znajduje się w stanie $\ket{\Psi_i}$ z prawdopodobieństwem $q_i$, $\sum_i q_i = 1$\\
    W mechanice Kwantowej prawdopodobieństwa, wartości oczekiwane liczymy zawsze jako $\ev{\hat{A}}{\Psi}$.
    Jeśli prawdopodobieństwo, że wynik pomiaru odpowiada stanowi $\ket{a}, \hat{A} = \ketbra{a}$.\\
    Dla niepełnej wiedzmy możemy liczyć prawdopodobieństwo: $\sum_i q_i \ev{\hat{A}}{\Psi} = \sum_i q_i \Tr (\hat{A} \ketbra{\Psi_i})$
    ...
    \linediv \\
    \emph{Definicja} \subind{Stany Mieszane}{Stan!Mieszany}\\
    Jeśli $\rho = \ketbra{\Psi}$ to mówimy, że stan jest \subind{czysty}{Stan!Czysty}, a jeśli nie, to $\rho$ nazywamy stanem mieszanym (czyli jeśli $\rho$ ma więcej niż jedną niezerową wartość własną).\\
    \linediv \\
    \emph{Przykład}\\
    Jeśli w double slit experiment nie mierzymy którą drogą leci:
    \[
        \ket{\Psi} = \frac{1}{\sqrt{2}} \qty(\ket{0} + \ket{1})    
    \]
    \[    
        \rho_\Psi = \ketbra{\Psi} = \frac12 \mqty[1 & 1\\ 1 & 1]
    \]
    Gdzie jedynka na w lewym dolnym rogu odpowiada za koherencję.\\
    Jeśli ktoś mierzył to, ale zapomniał powiedzieć nam, to mamy:
    \[
        \rho = \frac12 \ketbra{a} + \frac12 \ketbra{1} = \frac12 \mqty[1 & 0 \\ 0 & 1]    
    \]
\end{lecture}

\begin{lecture}{Do uzupełnienia}
